...
const invoiceSchema = new Schema(
  {
    ...
    returns: [{ type: Schema.Types.ObjectId, ref: 'Return', default: [] }],
    ...
    status: {
      ...
      default: 'issued', // Changed from 'draft' — an invoice created via the API is immediately issued
    },
...
// Remove manual isDeleted — softDeletePlugin handles all three fields consistently
...
invoiceSchema.index({ company: 1, isDeleted: 1 });
invoiceSchema.index({ company: 1, dueDate: 1, status: 1 }); // For overdue queries
...
invoiceSchema.pre('save', function (next) {
  if (PROTECTED_STATUSES.includes(this.status) && !this.isModified('status')) return next();
  if (!PROTECTED_STATUSES.includes(this.status)) {
    if (this.balanceDue <= 0 && this.amountPaid > 0) this.status = 'paid';
    else if (this.amountPaid > 0 && this.balanceDue > 0) this.status = 'partially_paid';
    else if (new Date() > this.dueDate && this.balanceDue > 0) this.status = 'overdue';
    else if (this.status === 'draft') this.status = 'issued'; // Auto-issue on creation
  }
  next();
});
